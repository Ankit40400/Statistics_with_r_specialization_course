...
#this will tell you the rows and columns in data
dim(sfo_feb_flights)
```
...
<li> Delta and United Airlines </li> <bold> - CORRECT ANSWER</bold>
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(medin = median(arr_delay), quantile_range = IQR(arr_delay)) %>% arrange(desc(quantile_range))
```
...
<ol>
<li> Mean would be more reliable as it gives us the true average. </li> <bold> -CORRECT </BOLD> 
